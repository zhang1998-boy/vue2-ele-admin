 ...
 * @LastEditTime: 2024-06-19 13:37:49
 * @FilePath: /gatherone_oms_fronted/src/views/Home/goodsStructure/index.vue
 * @Description: 设计工程填报 - 变洽签管理
-->
<template>
  <div class="table_container">
    <div class="info">
      <div class="rt-title">变洽签管理</div>
      <div class="info-btn">
    ...
    >
      ...
      />
      <el-table-column
        v-for="(item, index) in changeTableHeader"
        :key="index"
        :prop="item.prop"
        :label="item.label"
        :width="item.width"
...
import { changeTableHeader } from '@/utils/tableHeader'
...
export default {
  ...
  methods: {
    ...
    async getResturants() {
      ...
      let { data, retCode } = await getInfoList({}, 'changeOfNegotiation')
      if (retCode === '0') {
        this.tableData = data
      } else {
        this.tableData = []
