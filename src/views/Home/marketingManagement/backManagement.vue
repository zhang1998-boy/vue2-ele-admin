 ...
 * @LastEditTime: 2024-06-19 13:38:30
 * @FilePath: /gatherone_oms_fronted/src/views/Home/goodsStructure/index.vue
 * @Description: 营销填报 - 认签回达成
-->
<template>
  <div class="table_container">
    <div class="info">
      <div class="rt-title">认签回达成</div>
      <div class="info-btn">
    ...
    >
      ...
      />
      <el-table-column
        v-for="(item, index) in backDataTableHeader"
        :key="index"
        :prop="item.prop"
        :label="item.label"
        :width="item.width"
...
import { backDataTableHeader } from '@/utils/tableHeader'
...
export default {
  ...
  methods: {
    ...
    async getResturants() {
      ...
      let { data, retCode } = await getInfoList({}, 'houseSalesData')
      if (retCode === '0') {
        this.tableData = data
      } else {
        this.tableData = []
