 ...
 * @LastEditTime: 2024-06-19 13:38:39
 * @FilePath: /gatherone_oms_fronted/src/views/Home/goodsStructure/index.vue
 * @Description: 营销填报 - 货值结构
-->
<template>
  <div class="table_container">
    <div class="info">
      <div class="rt-title">货值结构</div>
      <div class="info-btn">
    ...
    >
      ...
      />
      <el-table-column
        v-for="(item, index) in goodsDataTableHeader"
        :key="index"
        :prop="item.prop"
        :label="item.label"
        :width="item.width"
...
import { goodsDataTableHeader } from '@/utils/tableHeader'
import { achievementImport, getInfoList } from '@/api/list'
export default {
  data() {
    return {
      goodsDataTableHeader,
      city: {},
      offset: 0,
      limit: 20,
      count: 0,
  ...
  methods: {
    ...
    async getResturants() {
      ...
      let { data, retCode } = await getInfoList({}, 'valueStructure')
      if (retCode === '0') {
        this.tableData = data
      } else {
        this.tableData = []
