...
<template>
  <div class="table_container">
    ...
    >
      ...
      />
      <el-table-column
        v-for="(item, index) in moneyTableHeader"
        :key="index"
        :prop="item.prop"
        :label="item.label"
        :width="item.width"
...
import { moneyTableHeader } from '@/utils/tableHeader'
...
export default {
  ...
  methods: {
    ...
    async getResturants() {
      ...
      let { data, retCode } = await getInfoList({}, 'performanceManagement')
      if (retCode === '0') {
        this.tableData = data
      } else {
        this.tableData = []
