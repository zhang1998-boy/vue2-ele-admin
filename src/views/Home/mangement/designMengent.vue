 ...
 * @LastEditTime: 2024-06-19 13:38:17
 * @FilePath: /zt-fill-web/src/views/Home/mangement/dataMengent copy.vue
 * @Description: 运营填报 - 在建规模
-->
<template>
  <div class="table_container">
    <div class="info">
      <div class="rt-title">在建规模</div>
      <div class="info-btn">
    ...
    >
      ...
      />
      <el-table-column
        v-for="(item, index) in designTableHeader"
        :key="index"
        :prop="item.prop"
        :label="item.label"
        :width="item.width"
...
<script>
import { designTableHeader } from '@/utils/tableHeader'
...
export default {
  ...
  methods: {
    ...
    async getResturants() {
      ...
      let { data, retCode } = await getInfoList({}, 'constructionScaleBuild')
      if (retCode === '0') {
        this.tableData = data
      } else {
        this.tableData = []
