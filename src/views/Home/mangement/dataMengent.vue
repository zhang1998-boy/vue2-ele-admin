<template>
  <div class="table_container">
    ...
    >
      ...
      />
      <el-table-column
        v-for="(item, index) in businessDataTableHeader"
        :key="index"
        :prop="item.prop"
        :label="item.label"
        :width="item.width"
...
import { businessDataTableHeader } from '@/utils/tableHeader'
import { achievementImport, getInfoList } from '@/api/list'
export default {
  data() {
    return {
      businessDataTableHeader,
      city: {},
      offset: 0,
      limit: 20,
      count: 0,
  ...
  methods: {
    ...
    // 获取表格数据
    async getResturants() {
      let { data, retCode } = await getInfoList({}, 'corporatePerformance')
      this.tableData = data
    },
