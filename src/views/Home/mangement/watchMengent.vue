 ...
 * @LastEditTime: 2024-06-19 13:38:25
 * @FilePath: /zt-fill-web/src/views/Home/mangement/dataMengent copy.vue
 * @Description: 运营填报 - 在建监管
-->
<template>
  <div class="table_container">
    <div class="info">
      <div class="rt-title">在建监管</div>
      <div class="info-btn">
    ...
    >
      ...
      />
      <el-table-column
        v-for="(item, index) in watchTableHeader"
        :key="index"
        :prop="item.prop"
        :label="item.label"
        :width="item.width"
...
import { watchTableHeader } from '@/utils/tableHeader'
...
export default {
  ...
  methods: {
    ...
    async getResturants() {
      ...
      let { data, retCode } = await getInfoList(
        {},
        'constructionScaleSupervision'
      )
      if (retCode === '0') {
        this.tableData = data
      } else {
