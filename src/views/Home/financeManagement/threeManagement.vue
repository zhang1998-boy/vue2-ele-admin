 ...
 * @LastEditTime: 2024-06-19 13:38:06
 * @FilePath: /gatherone_oms_fronted/src/views/Home/goodsStructure/index.vue
 * @Description: 财务管理 - 三费管理
-->
<template>
  <div class="table_container">
    <div class="info">
      <div class="rt-title">三费管理</div>
      <div class="info-btn">
    ...
    >
      ...
      />
      <el-table-column
        v-for="(item, index) in threeDataTableHeader"
        :key="index"
        :prop="item.prop"
        :label="item.label"
        :width="item.width"
...
import { threeDataTableHeader } from '@/utils/tableHeader'
import { achievementImport, getInfoList } from '@/api/list'
export default {
  data() {
    return {
      threeDataTableHeader,
      city: {},
      offset: 0,
      limit: 20,
      count: 0,
  ...
  methods: {
    ...
    async getResturants() {
      ...
      let { data, retCode } = await getInfoList({}, 't_expenses_management')
      if (retCode === '0') {
        this.tableData = data
      } else {
        this.tableData = []
