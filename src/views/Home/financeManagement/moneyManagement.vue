 ...
 * @LastEditTime: 2024-06-19 13:38:00
 * @FilePath: /gatherone_oms_fronted/src/views/Home/goodsStructure/index.vue
 * @Description: 财务管理 - 利润营收
-->
<template>
  <div class="table_container">
    <div class="info">
      <div class="rt-title">利润营收</div>
      <div class="info-btn">
    ...
    >
      ...
      />
      <el-table-column
        v-for="(item, index) in incomeTableHeader"
        :key="index"
        :prop="item.prop"
        :label="item.label"
        :width="item.width"
...
import { incomeTableHeader } from '@/utils/tableHeader'
...
export default {
  ...
  methods: {
    ...
    async getResturants() {
      ...
      let { data, retCode } = await getInfoList({}, 'revenueProfit')
      if (retCode === '0') {
        this.tableData = data
      } else {
        this.tableData = []
